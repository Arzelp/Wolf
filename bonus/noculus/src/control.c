...
** Last update Thu Dec 22 15:31:01 2016 Paskal Arzel
*/
...
void	shot(t_list *list)
{
  if (list->weapon.cooldown <= 0 && list->keys[BKS_SPACE])
    {
      list->weapon.firing = true;
      list->weapon.cooldown = SMG_CD;
    }
  else
    list->weapon.firing = false;
}
...
void	controls(t_list	*list)
{
  ...
  list->weapon.cooldown -= (list->weapon.cooldown <= 0) ? 0 : 1;
